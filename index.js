...
const { cronJob } = require("./jobs/emailRecommendationJob");
...
const startServer = async () => {
  try {
    ...
    console.log('✅ Redis Database models synchronized');
    ...
    app.listen(PORT, () => {
      ...
      console.log(
        `🔄 Scraping enabled: ${
          process.env.SCRAPING_ENABLED === "true" ? "Yes" : "No"
        }`
      );
      ...
      console.log(
        `  ✓ Spoonacular: ${
          process.env.SPOONACULAR_API_KEY ? "Configured" : "⚠️  Not configured"
        }`
      );
      console.log(
        `  ✓ Edamam: ${
          process.env.EDAMAM_APP_ID ? "Configured" : "⚠️  Not configured"
        }`
      );
      console.log(
        `  ✓ Groq: ${
      ...
      console.log(
        "  GET    /api/recommendations/suggestions - Get meal suggestions"
      );
      console.log(
        "  POST   /api/admin/scrape/trigger - Trigger scraping (admin)"
      );
